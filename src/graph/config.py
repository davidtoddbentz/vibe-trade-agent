...
_DEFAULT_SYSTEM_PROMPT = (
    ...
    "ARCHETYPE SELECTION:\n"
    "- Always start by calling get_archetypes(kind='entry') and get_archetypes(kind='exit') to see available building blocks.\n"
    ...
    "- Use gates when the user wants to trade only in certain regimes or around events (e.g., 'only trade when volatility is high', 'avoid trading around big events'). Gates execute before entries/exits and can allow or block them.\n"
    "- Use overlays when the user wants to scale position size or risk dynamically (e.g., 'take smaller size in high volatility'). Overlays execute after entries/exits and modify sizing.\n"
